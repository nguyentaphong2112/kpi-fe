...
export class WinComponent extends BaseListComponent<NzSafeAny> implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    super.ngOnInit();
    this.objFunctionWorkProcess = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.PERSONAL_WORK_PROCESS}`);
    this.objFunctionConcurrentProcess = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.PERSONAL_CONCURRENT_PROCESS}`);
    this.objFunctionContactProcess = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.PERSONAL_CONTRACT_PROCESS}`);
    this.objFunctionSalaryProgress = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.PERSONAL_INSURANCE_SALARY_PROCESS}`);
    this.objFunctionPositionSalary = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.PERSONAL_POSITION_SALARY_PROCESS}`);
    this.objFunctionAllowanceProcess = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.PERSONAL_ALLOWANCE_PROCESS}`);
    this.objFunctionPlanningAssignment = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.PERSONAL_PLANNING_ASSIGNMENTS}`);
    this.initAction();
    this.employeeId = 1;
  }
  ...
  doOpenFormCustom(type: string) {
    ...
    const formConfigMap: { [key: string]: any } = {
      ...
      concurrentProcess: {
        ...
        content: ConcurrentProcessFormComponent, config: UrlConstant.EMPLOYEES.PERSONAL
      },
      contactProcess: {
        title: 'hrm.staffManager.staffResearch.pageName.contractProcessInfo',
        content: ContractProcessFormComponent, config: UrlConstant.EMPLOYEES.PERSONAL
      },
      salaryProgress: {
        title: 'hrm.staffManager.staffResearch.pageName.insuranceSalaryInfo',
        content: IpsFormComponent, config: UrlConstant.EMPLOYEES.PERSONAL
      },
      positionSalary: {
        title: 'hrm.staffManager.staffResearch.pageName.positionSalaryInfo',
        content: PspFormComponent, config: UrlConstant.EMPLOYEES.PERSONAL
      },
      allowanceProcess: {
      ...
      planningAssignment: {
        ...
        content: PasFormComponent, config: UrlConstant.EMPLOYEES.PERSONAL
      }
    };
  ...
  doOpenFormEditCustom = (data: any) => {
    ...
    if (data.tableType === 'workProcess') {
      this.formConfig = {
        title: 'hrm.staffManager.staffResearch.pageName.workProcessInfo',
        content: WpsFormComponent, config: UrlConstant.EMPLOYEES.PERSONAL
      };
    }
    if (data.tableType === 'concurrentProcess') {
      this.formConfig = {
  ...
  deleteItemCustom = (data: any) => {
    if (data.tableType === 'workProcess') {
      ...
      this.deleteApi = (id: number | string) => this.workInfoService.deleteById(id.toString(), UrlConstant.EMPLOYEES.PERSONAL);
    }
    if (data.tableType === 'concurrentProcess') {
      this.key = 'concurrentProcessId';
      this.deleteApi = (id: number | string) => this.concurrentProcessInfoService.deleteById(id.toString(), UrlConstant.EMPLOYEES.PERSONAL);
    }
    if (data.tableType === 'contactProcess') {
      this.key = 'contractProcessId';
      this.deleteApi = (id: number | string) => this.contactInfoService.deleteById(id.toString(), UrlConstant.EMPLOYEES.PERSONAL);
    }
    if (data.tableType === 'salaryProgress') {
      this.key = 'insuranceSalaryProcessId';
      this.deleteApi = (id: number | string) => this.salaryProcessService.deleteById(id.toString(), UrlConstant.EMPLOYEES.PERSONAL);
    }
    if (data.tableType === 'positionSalary') {
      this.key = 'positionSalaryProcessId';
      this.deleteApi = (id: number | string) => this.positionSalaryProcessService.deleteById(id.toString(), UrlConstant.EMPLOYEES.PERSONAL);
    }

    if (data.tableType === 'allowanceProcess') {
      this.key = 'allowanceProcessId';
      this.deleteApi = (id: number | string) => this.allowanceProcessService.deleteById(id.toString(), UrlConstant.EMPLOYEES.PERSONAL);
    }

    if (data.tableType === 'planningAssignment') {
      this.key = 'planningAssignmentId';
      this.deleteApi = (id: number | string) => this.planningAssignmentService.deleteById(id.toString(), UrlConstant.EMPLOYEES.PERSONAL);
    }
  ...
  search(page?: number, type = '', isAlertModal = true) {
    ...
    if ((type === 'workProcess' || type === '') && this.objFunctionWorkProcess?.view) {
      this.workInfoService.getTableList(null, this.pagination.getCurrentPage()).subscribe(res => {
    ...
    if ((type === 'concurrentProcess' || type === '') && this.objFunctionConcurrentProcess?.view) {
      this.concurrentProcessInfoService.getTableList(null, this.pagination.getCurrentPage()).subscribe(res => {
    ...
    if ((type === 'contactProcess' || type === '') && this.objFunctionContactProcess?.view) {
      this.contactInfoService.getTableList(null, this.pagination.getCurrentPage()).subscribe(res => {
    ...
    if ((type === 'salaryProgress' || type === '') && this.objFunctionSalaryProgress?.view) {
      this.salaryProcessService.getTableList(null, this.pagination.getCurrentPage()).subscribe(res => {
    ...
    if ((type === 'positionSalary' || type === '') && this.objFunctionPositionSalary?.view) {
      this.positionSalaryProcessService.getTableList(null, this.pagination.getCurrentPage()).subscribe(res => {
    ...
    if ((type === 'allowanceProcess' || type === '') && this.objFunctionAllowanceProcess?.view) {
      this.allowanceProcessService.getTableList(null, this.pagination.getCurrentPage()).subscribe(res => {
    ...
    if ((type === 'planningAssignment' || type === '') && this.objFunctionPlanningAssignment?.view) {
      this.planningAssignmentService.getTableList(null, this.pagination.getCurrentPage()).subscribe(res => {
