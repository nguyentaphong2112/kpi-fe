...
export class HeaderInfoComponent extends BaseListComponent<NzSafeAny> implements OnInit, OnChanges {
  ...
  constructor(
    ...
    this.exportApi = (params: any) => this.employeeService.export(params, UrlConstant.EMPLOYEES.EXPORT + '/' + this.employeeId, false);
  }
  ...
  ngOnInit(): void {
    this.shareService.employee$.subscribe(emp => {
      if (emp?.employeeId) {
        ...
        this.getDataConfirm(this.employeeId);
      }
    });
  }
  ...
  getDataConfirm(employeeId: number) {
    this.employeeRequestService.getList(null, UrlConstant.EMPLOYEE_REQUESTS.GET_UPDATE_INFO + employeeId).subscribe(res => {
  ...
  deleteAvt() {
    this.avtIsLoading = true;
    this.subs.push(
      this.personalInfoService.deleteAvatar(this.employeeId, {}).subscribe(() => {
  ...
  handleOk(): void {
    ...
    this.employeeRequestService.update(this.dataEmployee, REQUEST_TYPE.DEFAULT, UrlConstant.EMPLOYEE_REQUESTS.UPDATE_EMPLOYEE + this.employeeId).subscribe(res => {
