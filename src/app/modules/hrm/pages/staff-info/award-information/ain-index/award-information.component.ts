...
export class AwardInformationComponent extends BaseListComponent<NzSafeAny> implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    super.ngOnInit();
    this.objFunctionAwardProcess = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.HR_AWARD_PROCESS}`);
    this.objFunctionDisciplineProcess = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.HR_DISCIPLINE_PROCESS}`);
    this.objFunctionEvaluationResult = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.HR_EVALUATION_RESULTS}`);
  ...
  search(page?: number, type = '', isAlertModal = true) {
    ...
    if ((type === 'award' || type === '') && this.objFunctionAwardProcess?.view) {
      this.awardProcessService.getAwardProcess(this.employeeId, this.pagination.getCurrentPage()).subscribe(res => {
    ...
    if ((type === 'discipline' || type === '') && this.objFunctionDisciplineProcess?.view) {
      this.disciplineProcessService.getDisciplineProcess(this.employeeId, this.pagination.getCurrentPage()).subscribe((res => {
    ...
    if ((type === 'evaluation' || type === '') && this.objFunctionEvaluationResult?.view) {
      this.evaluationResultService.getEvaluationResult(this.employeeId, this.pagination.getCurrentPage()).subscribe(res => {
  ...
  doOpenFormCustom(type: string) {
    const formConfigMap: { [key: string]: any } = {
      award: { title: 'hrm.staffManager.staffResearch.pageName.awardProcessInfo', content: ApsFormComponent },
      discipline: { title: 'hrm.staffManager.staffResearch.pageName.disciplineProcessInfo', content: DpsFormComponent },
      evaluation: { title: 'hrm.staffManager.staffResearch.pageName.evaluationResultInfo', content: ErsFormComponent }
    };
  ...
  deleteItemCustom = (data: any) => {
    if (data.tableType === 'award') {
      ...
      this.deleteApi = (id: number | string) => this.awardProcessService.deleteById(id.toString(), '/' + this.employeeId);
    }
    if (data.tableType === 'discipline') {
      this.key = 'disciplineProcessId';
      this.deleteApi = (id: number | string) => this.disciplineProcessService.deleteById(id.toString(), '/' + this.employeeId);
    }
    if (data.tableType === 'evaluation') {
      this.key = 'evaluationResultId';
      this.deleteApi = (id: number | string) => this.evaluationResultService.deleteById(id.toString(), '/' + this.employeeId);
    }
