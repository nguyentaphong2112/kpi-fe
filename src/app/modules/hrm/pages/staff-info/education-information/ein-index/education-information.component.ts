...
export class EducationInformationComponent extends BaseListComponent<NzSafeAny> implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    super.ngOnInit();
    this.objFunctionDegree = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.HR_EDUCATION_DEGREES}`);
    this.objFunctionCertificate = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.HR_EDUCATION_CERTIFICATES}`);
    this.objFunctionProcess = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.HR_EDUCATION_PROCESS}`);
    this.objFunctionPromotion = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.HR_EDUCATION_PROMOTIONS}`);
  ...
  doOpenFormCustom(type: string) {
    const formConfigMap: { [key: string]: any } = {
      degree: { title: 'hrm.staffManager.staffResearch.pageName.educationDegreesInfo', content: EdsFormComponent },
      certificate: {
        title: 'hrm.staffManager.staffResearch.pageName.educationCertificatesInfo',
        content: EceFormComponent
      },
      process: { title: 'hrm.staffManager.staffResearch.pageName.educationProcessInfo', content: EpsFormComponent },
      promotions: { title: 'hrm.staffManager.staffResearch.pageName.educationPromotionInfo', content: EpnFormComponent }
    };
  ...
  deleteItemCustom = (data: any) => {
    if (data.tableType === 'degree') {
      ...
      this.deleteApi = (id: number | string) => this.educationDegreesService.deleteById(id.toString(), '/' + this.employeeId);
    }
    if (data.tableType === 'certificate') {
      this.key = 'educationCertificateId';
      this.deleteApi = (id: number | string) => this.educationCertificatesService.deleteById(id.toString(), '/' + this.employeeId);
    }
    if (data.tableType === 'process') {
      this.key = 'educationProcessId';
      this.deleteApi = (id: number | string) => this.educationProcessService.deleteById(id.toString(), '/' + this.employeeId);
    }
    if (data.tableType === 'promotions') {
      this.key = 'educationPromotionId';
      this.deleteApi = (id: number | string) => this.educationPromotionService.deleteById(id.toString(), '/' + this.employeeId);
    }
  ...
  search(page?: number, type = '', isAlertModal = true) {
    ...
    if ((type === 'degree' || type === '') && this.objFunctionDegree?.view) {
      this.educationDegreesService.getEducationDegrees(this.employeeId, this.pagination.getCurrentPage()).subscribe(res => {
    ...
    if ((type === 'certificate' || type === '') && this.objFunctionCertificate?.view) {
      this.educationCertificatesService.getEducationCertificates(this.employeeId, this.pagination.getCurrentPage()).subscribe((res => {
    ...
    if ((type === 'process' || type === '') && this.objFunctionProcess?.view) {
      this.educationProcessService.getEducationProcess(this.employeeId, this.pagination.getCurrentPage()).subscribe(res => {
    ...
    if ((type === 'promotions' || type === '') && this.objFunctionPromotion?.view) {
      this.educationPromotionService.getEducationPromotions(this.employeeId, this.pagination.getCurrentPage()).subscribe(res => {
