...
export class PioIndexComponent extends BaseListComponent<NzSafeAny> implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    super.ngOnInit();
    this.objFunctionParticipation = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.HR_POLITICAL_PARTICIPATIONS}`);
  ...
  deleteItemCustom = (data: any) => {
    if (data.tableType === 'participation') {
      ...
      this.deleteApi = (id: number | string) => this.participationService.deleteById(id.toString(), '/' + this.employeeId);
    }
  ...
  search(page?: number, type = '', isAlertModal = true) {
    ...
    if ((type === 'participation' || type === '') && this.objFunctionParticipation?.view) {
      this.participationService.getParticipation(this.employeeId, this.pagination.getCurrentPage()).subscribe(res => {
  ...
  doOpenFormCustom(type: string) {
    const formConfigMap: { [key: string]: any } = {
      participation: { title: 'hrm.staffManager.staffResearch.pageName.participation', content: PonFormComponent }
    };
