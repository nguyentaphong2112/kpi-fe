...
@Component({
  ...
  templateUrl: './tkg-index.component.html',
  styleUrls: ['./tkg-index.component.scss']
})


export class TkgIndexComponent extends BaseListComponent<TimekeepingsModel> implements OnInit {
  serviceName = MICRO_SERVICE.ABS;
  objTimekeeping: AppFunction;
  ...
  constructor(
    ...
    this.objTimekeeping = this.sessionService.getSessionData(`FUNCTION_${FunctionCode.ABS_TIMEKEEPINGS}`);
  ...
  handleOk(){
    ...
    let data = CommonUtils.convertDataSendToServer(this.formTimekeeping.value)
    this.service.createOrImport(data, REQUEST_TYPE.DEFAULT).subscribe(res => {
    ...
    if (this.form.valid) {
      ...
      this.service.searchTimeKeeping(params, this.pagination.getCurrentPage()).subscribe(res => {
        ...
        if (res.code == HTTP_STATUS_CODE.SUCCESS) {
          ...
          const endWidths = ['100px', '150px', '250px'];
