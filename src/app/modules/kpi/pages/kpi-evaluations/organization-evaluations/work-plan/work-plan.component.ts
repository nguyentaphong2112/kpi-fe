...
export class WorkPlanComponent extends BaseFormComponent<NzSafeAny> implements OnInit {
  ...
  constructor(
    ...
    this.createApi = (body: OrganizationEvaluationsModel) => this.service.createOrImport(CommonUtils.convertDataSendToServer(body), REQUEST_TYPE.FORM_DATA, UrlConstant.ORGANIZATION_EVALUATION.WORK_PLANNING);
    this.updateApi = (body: OrganizationEvaluationsModel) => this.service.update(CommonUtils.convertDataSendToServer(body), REQUEST_TYPE.FORM_DATA, UrlConstant.ORGANIZATION_EVALUATION.WORK_PLANNING);
  }
  ...
  patchValueInit() {
    this.service.findOneById(this.route.snapshot.queryParams.organizationEvaluationId, UrlConstant.ORGANIZATION_EVALUATION.WORK_PLANNING).subscribe(res => {
      if (res.code === HTTP_STATUS_CODE.SUCCESS) {
        if (res.data.length > 0) {
          this.form.patchValue(res.data[0]);
          this.patchValue(res.data[0].content);
        }
      }
    });
  }
  ...
  override initForm() {
    this.form = this.fb.group({
      organizationWorkPlanningId: [null],
      organizationEvaluationId: [this.route.snapshot.queryParams.organizationEvaluationId],
  ...
  addChild = (item: FormGroup) => {
    ...
    item.controls['result'].setValue(null);
    item.controls['result'].clearValidators();
    item.controls['result'].setErrors(null);
        ...
        if (this.map.get(data.value.parentKey) == 0) {
          for (let i = 0; i < this.formArray.length; i++) {
            ...
            if (currentItem.key == data.value.parentKey) {
              ...
              this.formArray.at(i).get('result').setValue(null);
            }
          }
        }
      }
      ...
      if (this.map.get(data.value.parentKey) == 0) {
        this.formArray.controls.forEach(el => {
          if (el.value.key === data.value.parentKey) {
            el.get('result').setValidators(Validators.required);
            el.get('result').updateValueAndValidity();
          }
        });
      }
    }
  ...
  changeKHCT(name: string) {
    ...
    result = (length > 0 ? sum / length : 0).toFixed(2);
