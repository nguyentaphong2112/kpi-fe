...
export function totalPercentValidator(name: string, orgId = 0): ValidatorFn {
  return (control: AbstractControl): { [key: string]: any } | null => {
    ...
    formArray.controls.forEach(group => {
      let percent = 0;
      if (orgId == 1) {
        if (group.get('level')?.value == 1) {
          percent = +group.get(name)?.value || 0;
        }
      } else {
        percent = +group.get(name)?.value || 0;
      }
    ...
    return totalPercent != 100 ? { totalPercentExceeded: true } : null;
  };
}
...
export class EvaluationCriteriaComponent extends BaseFormComponent<NzSafeAny> implements OnInit, OnDestroy {
  ...
  getTotalResult(): number {
    return this.formArray.controls.reduce((sum, item) => sum + (parseFloat(item.get('result')?.value) || 0), 0);
  }
