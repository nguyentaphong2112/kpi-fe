...
export class IndicatorPopupComponent extends BaseFormComponent<NzSafeAny> implements OnInit {
  urlLoadOrganization = UrlConstant.INDICATOR_CONVERSION.ORGANIZATION_LIST;
  listOrgType = [];
  listJob = [];
  ...
  ngOnInit() {
    ...
    if (this.org || (this.data && this.data.length > 0)) {
      this.search(1, this.data);
    }
    this.jobsService.getList(null, UrlConstant.JOBS.VI_TRI_VIEC_LAM_SUB).subscribe(res => {
  ...
  initFormSearch() {
    this.form = this.fb.group({
      ...
      organizationId: [this.org ? this.orgId : 1, this.org ? null : Validators.required],
      orgTypeId: [null],
      name: [null]
    });
  }
