...
export class PesIndexComponent extends BaseListComponent<NzSafeAny> implements OnInit {
  functionCode = Constant.FUNCTION_CODE.PERSONAL_EVALUATIONS;
  ...
  constructor(
    ...
    super(injector);
    this.nameLocalSearch = 'search-provides-per-evaluation';
    this.nameLocalForm = 'provides-pes-form';
    this.initFormSearch();
    ...
    this.actionSchema = new ActionSchema({
      arrAction: [
        new ChildActionSchema({
          ...
          icon: 'setting',
          isShowFn: (evt: any) => evt.status === Constant.STATUS.PHE_DUYET && this.objFunction?.edit,
          function: (evt: any) => {
        ...
        new ChildActionSchema({
          ...
          function: (evt: any) => {
            this.navigateEdit(evt.employeeName, evt.employeeCode, evt.employeeEvaluationId,
              evt.employeeId, evt.evaluationPeriodId, true, null, evt.status, evt.isConcurrent);
          }
        }),
        // new ChildActionSchema({
  ...
  navigateEdit(employeeName?: string, employeeCode?: string, employeeEvaluationId?: number, employeeId?: number,
               evaluationPeriodId?: number, isDetail = null, isAdjust = null, status = null, isConcurrent = null) {
    this.router.navigate(['/kpi/kpi-provides/personal-evaluations/form'],
      {
        queryParams: {
          employeeName,
          employeeCode,
  ...
  // updateStatusById(id: number, status: string, content?: string) {
  //   this.service.update({
  //     id: id,
  //     status: status,
  //     content: content
  //   }, REQUEST_TYPE.FORM_DATA, UrlConstant.EMPLOYEE_EVALUATION.STATUS).subscribe(res => {
  //     if (res.code === HTTP_STATUS_CODE.SUCCESS) {
  //       this.toast.success(this.translate.instant('common.notification.updateSuccess'));
  //       this.search();
  //     }
