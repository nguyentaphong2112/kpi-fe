...
export class WorkPlanComponent extends BaseFormComponent<NzSafeAny> implements OnInit {
  ...
  isVisible = false;
  valueSelect: any;
  urlLoadData = UrlConstant.ORGANIZATION_EVALUATION.LIST + '?type=DON_VI';
    ...
    this.service.findOneById(this.route.snapshot.queryParams.organizationEvaluationId, UrlConstant.ORGANIZATION_EVALUATION.WORK_PLANNING).subscribe(res => {
      if (res.code === HTTP_STATUS_CODE.SUCCESS) {
        if (res.data.length > 0) {
          ...
          this.adjustReason = res.data[0].adjustReason;
      ...
      profile.get('stepTwo')?.valueChanges?.pipe(distinctUntilChanged()).subscribe((value: any) => {
        if (value) {
          if (!isNaN(value) && !isNaN(profile.get('stepOne').value as any)) {
            profile.get('fullYear').setValue(parseFloat((profile.get('stepOne').value as any ?? '0')) + parseFloat(value));
          } else {
            profile.get('fullYear').setValue(null);
          }
          profile.get('fullYear').setValidators(Validators.required);
