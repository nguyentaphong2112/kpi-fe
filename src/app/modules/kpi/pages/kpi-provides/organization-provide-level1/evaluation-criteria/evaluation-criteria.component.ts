...
export class EvaluationCriteriaComponent extends BaseFormComponent<NzSafeAny> implements OnInit {
  ...
  constructor(
    ...
    this.isPage = true;
    this.initDataSelect();
    this.findOneById = (id) => this.service.findOneById(id, UrlConstant.ORGANIZATION_EVALUATION.INDICATOR_LEVEL1);
  ...
  patchValue() {
    this.service.findOneById(this.route.snapshot.queryParams.organizationEvaluationId, UrlConstant.ORGANIZATION_EVALUATION.INDICATOR_LEVEL1).subscribe(res => {
  ...
  openAssignmentPopup(data: NzSafeAny) {
    const index = this.formArray.controls.findIndex((it: NzSafeAny) => it.value.indicatorId == data.indicatorId);
    this.modalRef = this.modal.create({
