...
export class ValidationService {
  ...
  public static atLeastOneRequiredValidator(): ValidatorFn {
      ...
      if (group instanceof FormGroup) {
        ...
        for (const key in controls) {
          if (controls.hasOwnProperty(key)) {
            if (controls[key].value !== null && controls[key].value !== '' && controls[key].value.length !== 0) {
        ...
        return hasNonEmptyValue ? null : { atLeastOneRequired: true };
      }
      return null;
    };
  }

  public static maxGreaterThanMinValidator(): ValidatorFn {
    return (group: AbstractControl): ValidationErrors | null => {
      const minValue = group.get('minValue')?.value;
      const maxValue = group.get('maxValue')?.value;
      const maxComparison = group.get('maxComparison')?.value;
      const minComparison = group.get('minComparison')?.value;
      if (maxComparison === 'EQUAL' || minComparison === 'EQUAL') {
        return null;
      }

      if (this.isValid(minValue) && this.isValid(maxValue) &&
        ((maxValue == minValue && !(maxComparison === 'LESS_THAN_EQUAL' && minComparison === 'GREATER_THAN_EQUAL'))
          || (+maxValue < +minValue))) {
  ...
  public static minValueAndComparisonValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const minValue = control.get('minValue');
      const minComparison = control.get('minComparison');
      if (this.isValid(minValue?.value) && !minComparison?.value) {
        minComparison.setErrors({ required: true });
        return null;
      } else if (!this.isValid(minValue?.value) && minComparison?.value) {
        minValue.setErrors({ required: true });
        return null;
      } else {
        minValue?.setErrors(null);
        minComparison?.setErrors(null);
        return null;
      }
    };
  }

  public static maxValueAndComparisonValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const maxValue = control.get('maxValue');
      const maxComparison = control.get('maxComparison');
      if (this.isValid(maxValue?.value) && !maxComparison?.value) {
        maxComparison.setErrors({ required: true });
        return null;
      } else if (!this.isValid(maxValue?.value) && maxComparison?.value) {
        maxValue.setErrors({ required: true });
        return null;
      } else {
        maxValue?.setErrors(null);
        maxComparison?.setErrors(null);
        return null;
      }
    };
  }
  ...
  /**
   * Validate control number not small than the other
   * @param targetKey: any
   * @param toMatchKey: any
   */
  public static notLessThanNumber(targetKey: string, toMatchKey: string, labelMatchCode: string): ValidatorFn {
    return (group: FormGroup): { [key: string]: any } => {
      const target = group.controls[targetKey];
      const toMatch = group.controls[toMatchKey];
      // neu mot trong hai truong null thi chuyen sang require
      ...
      // dieu kien kiem tra
      if (target.hasError('notLessThanNumber')) {
        target.setErrors(null);
        target.markAsUntouched();
      }
      ...
      if (targetValue || targetValue === 0 && toMatchValue) {
        ...
        // set equal value error on dirty controls
        if (!isCheck && target.valid && toMatch.valid) {
          target.setErrors({ notLessThanNumber: { dateNotBefore: labelMatchCode } });
          target.markAsTouched();
        }
      }
      return null;
